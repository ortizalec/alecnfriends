...
export default function MemoryHome() {
  ...
  const [boardSize, setBoardSize] = useState('4x5')
  ...
  const handleStartGame = async () => {
    ...
    setError('')
    try {
      const result = await api.createMemoryGame(selectedFriend, boardSize)
      setShowNewGameModal(false)
      setSelectedFriend(null)
      navigate(`/memory/${result.game.id}`)
    } catch (err) {
      setError(err.message)
    } finally {
      setCreating(false)
  ...
  const GameCard = ({ game, showStatus }) => {
    ...
    const theirScore = game.player1_id === user?.id ? game.player2_score : game.player1_score

    return (
      <li
        className="game-card"
        onClick={() => navigate(`/memory/${game.id}`)}
      >
        <div className="game-card-main">
          ...
          <span className="game-card-score text-muted" style={{ fontSize: '0.75rem' }}>
            {myScore}-{theirScore} | {game.board_size}
          </span>
        </div>
            ...
            <div className="modal">
              ...
              {friends.length === 0 ? (
                ...
                <>
                  ...
                  <p className="modal-subtitle mt-2">Board Size</p>
                  <div className="option-buttons">
                    {['4x5', '6x6', '8x8'].map((size) => (
                      <button
                        key={size}
                        className={`option-btn ${boardSize === size ? 'selected' : ''}`}
